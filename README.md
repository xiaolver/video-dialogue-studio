...
如果视频没有公开字幕，助手会下载最佳音频，再用 `ffmpeg` 压缩为 16 kHz、单声道、24 kbps 的 MP3，交给 OpenAI `gpt-4o-mini-transcribe` 忠实转写。若本机助手不可用，系统依次回退到 Cloudflare Worker 直连 YouTube、Webshare 代理池；云端回退只能读取公开字幕，不能完成无字幕音频转写。

## 2. 如何调用 OpenAI GPT 并实现流式输出

Worker 将视频标题、字幕语言、字幕正文和生成要求组成提示词，请求 OpenAI Responses API，并使用 `stream: true` 接收 SSE。服务端处理 `response.output_text.delta` 等事件，再转换成 `meta`、`delta`、`done`、`error` 四类 NDJSON 事件，通过 `ReadableStream` 立即返回浏览器。
...
用户点击某章的 5W1H 按钮时，前端只提交 `generationId` 和 `sectionIndex`，不会重新上传整篇文章。服务端根据这两个字段读取已保存的完整上下文，请求 OpenAI Structured Outputs 按 JSON Schema 返回 `who`、`what`、`when`、`where`、`why`、`how` 六个字段，并按章节缓存结果，避免重复调用。

## 5. 主要工程取舍和亮点

- **本机提取、云端生成**：Cloudflare 数据中心 IP 容易触发 YouTube 验证，而且 Worker 不能直接运行 `yt-dlp` 和 `ffmpeg`。因此由本机处理媒体，Worker 负责中继、状态保存和 OpenAI 调用。
...
- **结构化流式协议**：OpenAI Responses API 的 SSE 在服务端转换成简单的 NDJSON 事件，前端状态清晰，也便于处理错误和完成信号。
